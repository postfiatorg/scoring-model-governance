...
- the exam corpus,
- the grading prompt — judge-independent, because any pool member may end up grading with it — together with the grading output schema, its parser, and the grade-aggregation rule (see Grade), so grades are as mechanically checkable as scores; the governance counterparts of the scoring prompt, response parser, and score formula the scoring pipeline already publishes,
...
The drawn judge grades each survivor's outputs against the frozen grading prompt, one corpus item at a time: a grading request carries the exam input, one candidate's answer, and nothing that identifies which candidate produced it — a judge cannot favor a model it recognizes, and grades are absolute against the prompt's rubric rather than comparisons within a pool, so they stay comparable across rounds. Per-item grades are banded — 0–100 in multiples of 5 — because finer per-item precision is noise, not judgment: the scoring prompt's own history showed that one-point distinctions do not survive repetition while banded ones do. A survivor's final grade — and that grade is the ranking — is computed in code as the unweighted mean of its per-item grades, on the 0–100 scale with one decimal place: the same split between bounded model judgment and deterministic arithmetic that the scoring pipeline's score formula applies. Cost and latency (GPU seconds, wall-clock per round) and operational feasibility (GPU class, cold-start behavior, the cost a validator operator bears to run it) are measured and published alongside the results for operators and future preference voting, but they are not combined into a weighted formula.
