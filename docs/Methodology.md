...
1. **Pull the open-weight models from LiveBench.** LiveBench is the pool's single source: its question set is regularly refreshed to resist contamination, its answers are scored against objective ground truth rather than a judge's preference, and one leaderboard keeps every refresh auditable. The leaderboard only sources candidates — the governance round does the real evaluation — so a hand-built aggregate across benchmark sites would add a formula to defend without adding signal. Models are ordered by our criteria through the leaderboard's own filters. A refresh reads exactly one leaderboard release, chosen deterministically: the question-set rotation that keeps LiveBench contamination-resistant also restarts its evaluated-model list, so the newest release can be too sparse to source a pool. The refresh uses the latest release that can supply a viable pool — at least two challengers surviving all five rules of this refresh, joining the incumbent the pool always carries — and otherwise falls back one release at a time; the release used and the reason for any fallback are recorded in the published refresh record. If no release can supply a viable pool, the refresh publishes that finding and the pool stands unchanged. Blocklisted revisions (see below) are skipped: the slot passes to the model's next non-blocked revision or family sibling, or to the next model on the leaderboard. A model whose weights cannot be resolved to a concrete open artifact — a pinned revision of a publicly hosted weight repository — is skipped the same way: a candidate the pool cannot pin is a candidate no verifier could reproduce.
2. **Resolve each model to one deployable artifact.** A leaderboard entry is a name; what enters the pool is a specific precision variant, chosen by a fixed preference order: the vendor's official FP8 release when one exists as a pinned revision of a publicly hosted repository, otherwise the vendor's own full-precision weights when they fit under the rule below, otherwise the model is not eligible for the refresh — its slot passing on like any skip above. An artifact on the blocklist is treated as absent from this order. Community-made quantizations are never eligible: their provenance cannot be audited, which a public governance record cannot absorb. The chosen artifact — repository, pinned revision, precision — is the candidate's identity, and fit checks, runtime profiles, and blocklist entries all bind to it.
3. **Keep only models whose thinking mode can be disabled.** Production serves the scoring model with thinking off — the cross-run determinism and output-format discipline commit-reveal verification depends on — so a model that can only serve with thinking enabled can never serve scoring. Plain non-thinking models and hybrids with a thinking toggle qualify; thinking-only models do not, and a model whose thinking behavior cannot be established from its public chat template is ineligible until it can — recorded as unknown, never guessed. Two conventions follow. A hybrid scored on the leaderboard with thinking enabled is examined with thinking disabled: the leaderboard only sources candidates, and the exam evaluates the configuration that would actually serve. And when a leaderboard lists both variants of the same weights, the non-thinking key is the one mapped.
4. **Keep only single-GPU models.** A model must fit on one GPU — a modest one or the strongest available, but exactly one — with at least 10% memory headroom. Single-GPU inference is the profile production determinism is proven on, so a model that needs more can never serve scoring.
5. **Deduplicate families.** One model per family, keeping the best-ranked one. Pool members judge each other, and distinct families remove same-family judging bias.

These rules select the challengers. The incumbent — the model currently serving scoring — is a pool member by right: it is exempt from the leaderboard, artifact, thinking-mode, and family rules (its serving artifact, thinking-disabled serving, and single-GPU fit are already proven in production), and no refresh can remove it. When the incumbent's own family produces a better-ranked successor, that successor takes the family's challenger slot and the pool carries both — refusing the pairing would lock out the most likely upgrade path, and the residual same-family judging bias is absorbed by the incumbent margin (see Decide).
...
5. Grade              a mechanical checker and the drawn judge find defects;
                      a code formula turns them into grades
...
- the exam corpus,
- the grading prompt — judge-independent, because any pool member may end up grading with it — together with the judge defect schema and its parser, the mechanical checker's per-scoring-prompt-version rules, and the versioned grade formula (see Grade), so grades are as mechanically checkable as scores; the governance counterparts of the scoring prompt, response parser, and score formula the scoring pipeline already publishes,
...
The judge is held to the same mechanical bar as any candidate. If it fails to deploy and serve on its pinned profile, its output does not parse under the frozen judge defect schema, or its grading runs are not bit-identical across the frozen repeat count, the frozen redraw ordering applies: the same ledger hash maps to the next challenger, repeating as needed. A judge drawn by redraw has already sat the exam; its exam outputs stay in the round record but are discarded from the competition — like any judge, it cannot win. A failed judge sits out the rest of the round entirely and enters the blocklist when the round closes. If the redraws exhaust the challengers, the round is abandoned — the incumbent stays, and the failed judges' blocklist entries are the round's only outcome.
...
Grading splits three ways — the same split between bounded model judgment and deterministic arithmetic that the scoring pipeline's score formula applies, carried one level further: a mechanical checker computes every defect with a closed-form right answer, the drawn judge finds the defects only language understanding can detect, and a versioned grade formula turns both defect lists into the grade. The judge never emits a number.

The mechanical checker runs in code over each (corpus item, survivor answer) pair: validators with identical evidence in a dimension scored differently, strictly better evidence scored strictly worse, the numeric rules of the item's scoring-prompt version broken (ceilings, banding, required penalties), and structural failures (missing or invented validator entries) — driven by a hand-curated rules table with one frozen row per published scoring-prompt version. Checker-owned defect kinds are exclusive to it: the judge is never asked about them, so the two defect lists cannot overlap.

The drawn judge examines each survivor's outputs against the frozen grading prompt, one corpus item at a time: a grading request carries the exam input, one candidate's answer, and nothing that identifies which candidate produced it — a judge cannot favor a model it recognizes. Its output is a set of structured defect objects under the frozen judge defect schema — claims the evidence contradicts, decisive evidence the answer's text ignores, report mismatches, grader-directed subversion — each citing the quoted claim and the contradicting evidence, with every check stating an explicit none-found outcome when it finds nothing; no grade, no counts, no severity anywhere in the output.

Per-item grades are computed by the grade formula from the two defect lists: classification by declared thresholds, counting, and band selection — 0–100 in multiples of 5, because finer per-item precision is noise, not judgment: the scoring prompt's own history showed that one-point distinctions do not survive repetition while banded ones do. Grades stay absolute — a deterministic function of the defects found under a fixed rubric, never comparisons within a pool — so they remain comparable across rounds. A survivor's final grade — and that grade is the ranking — is the unweighted mean of its per-item grades, on the 0–100 scale with one decimal place. Cost and latency (GPU seconds, wall-clock per round) and operational feasibility (GPU class, cold-start behavior, the cost a validator operator bears to run it) are measured and published alongside the results for operators and future preference voting, but they are not combined into a weighted formula.

The judge runs with the same discipline as any pool member: pinned revision, pinned SGLang image, deterministic profile, defect findings emitted under the frozen schema and parsed by the frozen parser. It examines only what cannot be checked mechanically — the language of the scoring reasoning. Because the checker and formula are pure code, judging is deterministic, and every artifact is pinned, anyone can re-run this round's grading and get identical defect sets and identical grades — and can re-grade any past round under any judge offline, so judge rotation never erases cross-round comparability.
